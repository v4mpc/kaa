...
{'variable':country,'key':'country'},{'variable':fuel,'key':'fuel'},{'variable':engine,'key':'engine'},
]
# car_properties={
#     'make':'TOYOTA',
#     'model':'rumion',
...
def get_model_value(dict_of_features,search):
    ...
    for feature in dict_of_features:
        ...
        score[feature['value']]=0
        for x in ls_features:
            if x.lower() in search:
                score[feature['value']]=score[feature['value']]+1
    
    sorted_x = sorted(score.items(), key=lambda kv: kv[1],reverse = True)    
    match=sorted_x[0]
    if match[1]==0:
...
def get_feature_value(html,feature,feature_variable):
    dict_of_features=extract_select(html,feature_variable)
    if feature_variable==model:
       return get_model_value(dict_of_features,feature)
    else:
...
def prepare_form_data(feature_variable,view_state_value,event_validation_value,make_value,model_value=0,year_value=0,country_value=0,fuel_value=0,engine_value=0):
    search=''
    if engine_value:
        search='Calculate'
    return {
...
def get_record_details(resp,car):
    ...
    td_values['grandtotal']=add_commas(float(car['price'])+float(td_values['totaltaxes'].replace('\n','').replace(',','')))
...
# refactor
    ...
    model_value=get_feature_value(resp2.text,car_properties['model'],model)   
    ...
    form_data=prepare_form_data(model,viewstate_value,eventvalidation_value,make_value,model_value,year_value,country_value)
    ...
    form_data=prepare_form_data(model,viewstate_value,eventvalidation_value,make_value,model_value,year_value,country_value,fuel_value)
...
# end refactor
    form_data=prepare_form_data(model,viewstate_value,eventvalidation_value,make_value,model_value,year_value,country_value,fuel_value,engine_value)
